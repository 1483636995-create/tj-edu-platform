...
- `db:seed` 可重复执行，初始化示范机构、九大学科、六个年级、天津 16 个区、示例知识点、4 份试卷、4 道题目、2 名教师、3 名学生、暑期课表、4 节演示课、3 份学生档案、4 条错题记录、2 条文件素材和 1 份讲义草稿。
- seed 同时创建本地管理员，默认账号为 `admin@tj-edu.local`，默认密码为 `ChangeMe123!`。
...
- `JWT_SECRET` 至少 32 个字符，生产环境必须使用密钥管理服务生成和注入。
- `JWT_EXPIRES_IN_SECONDS` 控制访问 Token 有效期，本地默认 8 小时。
- `SEED_ADMIN_EMAIL` 和 `SEED_ADMIN_PASSWORD` 仅控制开发 seed 管理员。
- `NEXT_PUBLIC_API_BASE_URL` 指定 Web 调用的 API 地址。
- `FILE_STORAGE_ROOT` 指定开发环境文件上传目录，默认使用仓库根目录的 `uploads/`，该目录不会提交到 Git。
- 文件存储通过服务接口隔离；对象存储变量为后续接入 MinIO 或云存储预留。

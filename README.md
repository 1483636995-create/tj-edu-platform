...
- 课节表单支持班课与一对一、教师、学生、教室和时间段管理，并展示明确的冲突提示。
- 学生档案页已连接真实 API，提供学生筛选、详情查看、学习记录、错题本和教师备注编辑。
- 错题本支持按题目、知识点、关联课节展示，并可在页面内更新新错题、复习中、已掌握状态。
- 讲义页已连接真实 API，支持讲义草稿列表、创建编辑、选择知识点/题目/文件素材和结构预览。
...
- 已提供课表筛选项、周期创建、课节列表、详情、创建和修改接口。
- 排课时会检查教师、教室和学生时间重叠，取消状态的课节不参与冲突计算。
- 已提供学生筛选项、分页列表、详情、档案备注更新和错题掌握状态更新接口。
- 学生档案 API 按当前机构隔离，学生角色只能访问自己的档案和错题。
- 已提供讲义素材筛选、草稿分页列表、详情、创建和修改接口。
- 讲义草稿 API 按当前机构隔离，素材选择支持公共题库、本机构题目和本机构文件。
- 已配置 CORS，支持从 Web 后台访问 API。
- 已形成 AI/OCR 技术方案，明确主 API、智能服务、异步队列、人工审核和隐私合规边界。
...
- 已提供 PostgreSQL 迁移和可重复执行的 seed，初始化管理员、九大学科、六个年级、天津 16 个区、示例知识点、4 份试卷、4 道题目、2 名教师、3 名学生、4 节演示课、3 份学生档案、4 条错题记录、2 条文件素材和 1 份讲义草稿。
...
- 已提供天津九大学科、天津区域、用户角色、认证会话、题库、文件库、课表、学生档案、错题本、讲义草稿响应和健康检查相关类型。
...
2. 已实现登录权限、题库筛选、文件上传下载、课表、学生档案、错题本和讲义制作基础版。
3. 实现知识点组题、更完整的教师端一生一案和讲义导出。
